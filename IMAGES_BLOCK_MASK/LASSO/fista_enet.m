function [beta1 t costs] =...
    fista_enet(X, y, lambda_1, lambda_2, tol_nepio, maxiter_nepio, Li, alpha0)
...
beta0 = zeros(n,1);
theta0 = 1;
costs = zeros(maxiter_nepio,1);

for t = 1:maxiter_nepio
    % GRADIENT STEP
    ...
    if t > 1 && abs(costs(t-1)-costs(t))/costs(t-1) < tol_nepio
       costs(t+1:end) = [];
       break;
    end
