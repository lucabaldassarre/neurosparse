function compute_results_atlas_LS

p = 0.5;

% Load models
load('LS_block_mask.mat','alpha','ns');
...
for ks = 1:ns
   % Compute l1 norm
   l1_norm = norm(alpha(ks,:),1);
   % Compute relative frequencies for each region
   for r = 1:R
      region_value = regions(r);
      atlas_hist(ks,r) = sum(abs(alpha(ks,ordered_atlas == region_value)))/l1_norm;
   end  
   clear l1_norm   
end
