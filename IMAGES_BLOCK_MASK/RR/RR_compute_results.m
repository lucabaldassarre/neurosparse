...
% Get data
for kloo = 1:ns
   fload = sprintf('RESULTS_TRAIN/RR_loo_%d_p_%g_perc_%1.1f.mat', kloo, p_mask, perc*100);
   load(fload,'err_test_accuracy','alpha_accuracy','err_test_dist_OC','alpha_dist_OC','err_test_dist_corr','alpha_dist_corr','idx');
   
   % ACCURACY
   % Get thresholded solution
   alpha_t = threshold_solutions(alpha_accuracy);
   alpha_tot_accuracy(kloo,idx) = alpha_t;
   support_accuracy{kloo} = find(alpha_t);
   sparsity_accuracy(kloo) = numel(idx)/n;
   err_loo_accuracy(kloo) = err_test_accuracy;
   
   % DIST OC
   % Get thresholded solution
   alpha_t = threshold_solutions(alpha_dist_OC);
   alpha_tot_dist_OC(kloo,idx) = alpha_t;
   support_dist_OC{kloo} = find(alpha_t);
   sparsity_dist_OC(kloo) = numel(idx)/n;
   err_loo_dist_OC(kloo) = err_test_dist_OC;
   
   % DIST corr
   % Get thresholded solution
   alpha_t = threshold_solutions(alpha_dist_corr);
   alpha_tot_dist_corr(kloo,idx) = alpha_t;
   support_dist_corr{kloo} = find(alpha_t);
   sparsity_dist_corr(kloo) = numel(idx)/n;
