...
for ks = 1:ns
   ...
   for ksv = 1:nsv
      fload = sprintf('RESULTS_VAL/RR_loo_%d_val_%d_p_%g_perc_%1.1f.mat', ks, ksv, p_mask, perc*100);
      load(fload,'err_val','alpha','idx');
      alpha_tot(ksv,idx,:) = alpha;
   ...
   [min_mean_err_loo, idx_best_regpar_accuracy] = min(mean_err_loo); %#ok<*ASGLU>
   ...
   fsave = sprintf('RESULTS_VAL/RR_loo_errs_%d_perc_%1.1f.mat',ks, perc*100);
   save(fsave);
