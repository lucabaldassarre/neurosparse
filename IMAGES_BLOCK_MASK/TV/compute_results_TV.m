...
% Get data
for kloo = 1:ns
   fload = sprintf('RESULTS_TRAIN/TV_loo_%d_p_%g.mat', kloo, p);
   load(fload,'err_test_accuracy','alpha_accuracy','err_test_dist_corr','alpha_dist_corr');
   
   % ACCURACY
   % Get thresholded solution
   n = size(alpha_accuracy,1);
   alpha_t = threshold_solutions(alpha_accuracy);
   alpha_tot_accuracy(kloo,:) = alpha_accuracy;
...
fsave = 'RESULTS_TRAIN/TV_results';
