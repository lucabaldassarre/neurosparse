...
alpha = cell(ns,1);
alpha_thresh = cell(ns,1);
time = zeros(ns,1);
err_class = zeros(ns,1);
err_class_thresh = zeros(ns,1);
idx_n = cell(ns,1);
...
for ks = 1:ns
   ...
   alpha_thresh{ks} = alpha{ks};
   [a b] = sort(abs(alpha{ks}));
   c = cumsum(a)/sum(a);
   d = find(c > threshold,1);
   alpha_thresh{ks}(b(1:d)) = 0;
   yprev = sign(Xts*alpha{ks});
   err_class_thresh(ks) = sum(1-yprev.*yts)/(2*size(yts,1));
   
end

clear X Y Xtr ytr Xts yts stds means yprev ks ans K idx_sorted p df t
save LS_ttest_10_perc_proper